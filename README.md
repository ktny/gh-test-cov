...
2. `.github/workflows/coverage.yml` は `workflow_run` トリガーで上記 2 つのワークフロー完了を監視します。`actions/github-script` で「対象のコミット (head_sha) で両方とも成功しているか」を確認し、揃っていない場合はその実行をスキップします。揃ったタイミングの実行では `actions/download-artifact@v4` の `run-id` 指定で両ワークフローのアーティファクトを取得し、octocov を central モード (`.octocov.central.yml`) → 通常モード (`.octocov.yml`) の順に 1 度だけ実行します。
...
- `test-api.yml` と `test-web.yml` の両方が成功したコミットだけ `coverage.yml` が実際に処理を行うため、octocov は 1 度だけ実行されます (`workflow_run` のうち片方が完了したタイミングでは「もう一方が未完了」と判断してスキップします)。
